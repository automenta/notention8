...
import { TagIcon, PlusIcon, TrashIcon, CheckIcon, XIcon, PencilIcon } from '../icons';
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-3 custom-scrollbar">
            {(isAdding || editingIndex !== null) && (
                <div className="bg-gray-800 p-3 rounded-md border border-blue-500/50 space-y-3 animate-fade-in shadow-lg">
                    <div className="text-xs font-bold text-blue-400 uppercase tracking-wider mb-1">
                        {isAdding ? 'New Property' : 'Edit Property'}
                    </div>
                    <input
                    ...
                    <input
                        className="w-full bg-gray-900 border border-gray-700 rounded px-2 py-1.5 text-sm text-white placeholder-gray-500 focus:border-blue-500 outline-none"
                    ...
                    <div className="flex justify-end gap-2 pt-1">
                        <button onClick={cancelEdit} className="p-1 text-gray-400 hover:text-white hover:bg-gray-700 rounded">
                            <XIcon className="w-4 h-4"/>
                        </button>
                        <button onClick={handleSave} className="p-1 text-green-500 hover:text-green-400 hover:bg-gray-700 rounded">
            ...
            {properties.map((prop, idx) => (
                ...
                >
                    <div className="flex justify-between items-start mb-1">
                        <div className="text-xs text-blue-400 font-mono font-bold truncate pr-6" title={prop.key}>
                            {prop.key}
                        </div>
                        <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity bg-gray-800 rounded">
                             <button onClick={() => startEdit(prop, idx)} className="p-1 hover:text-yellow-400 text-gray-400">
                                <PencilIcon className="w-3 h-3" />
                            </button>
                            <button onClick={() => handleDelete(prop)} className="p-1 hover:text-red-400 text-gray-400">
            ...
            {properties.length === 0 && !isAdding && (
                <div className="text-center text-gray-500 text-sm py-8 flex flex-col items-center gap-2 opacity-60">
                    ...
                    <p className="text-xs">Type <code>[key:val]</code> in the editor or add one manually.</p>
                </div>
            )}
        </div>
    </div>
