...
export class MatchingService {
  ...
  normalizeTerm(term: string): string {
    ...
    // Remove common punctuation to ensure cleaner matching
   ...
   *
   * Score = (Satisfied Constraints / Total Constraints) * Target Priority
   */
  matchNotes(request: Note, offer: Note): MatchResultDetails {
    ...
    // Weight by priority of the matched note (offer)
    // Default priority is 1.0 if undefined
   ...
   */
  calculateMatchScore(note1: Note, note2: Note): number {
      const semanticScore = this.calculateSemanticOverlap(note1, note2);
      const priority = note2.priority ?? 1.0;
      return semanticScore * priority;
  }
...
// Standalone functions for easier import/usage
export const calculateMatchScore = (note1: Note, note2: Note) => matchingService.calculateMatchScore(note1, note2);
